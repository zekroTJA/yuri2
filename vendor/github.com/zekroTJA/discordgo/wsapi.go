...
func (s *Session) ChannelVoiceJoinManual(gID, cID string, mute, deaf bool) (err error) {
	...
	// MODIFICATION BY github.com/zekroTJA
	// This makes that a nil pointer is passed to voiceChannelJoinData instead
	// of an empty string when the function actually is getting passed an empty
	// string which will force the voice connection to disconnect.
	pcID := &cID
	if cID == "" {
		pcID = nil
	}
	...
	// Send the request to Discord that we want to join the voice channel
	data := voiceChannelJoinOp{4, voiceChannelJoinData{&gID, pcID, mute, deaf}}
	s.wsMutex.Lock()
	err = s.wsConn.WriteJSON(data)
	s.wsMutex.Unlock()
	if err != nil {
